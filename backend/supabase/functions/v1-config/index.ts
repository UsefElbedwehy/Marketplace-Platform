// GET/PATCH /v1/config — contract/openapi/v1/openapi.yaml's `getConfig`
// operation, plus the Config Studio's publish path (RLS-gated to admins).
// Deployable standalone (Supabase Edge Function) or imported by
// backend/scripts/serve-local.ts for local dev — see the import.meta.main guard.
...
import { fetchActiveConfig, publishConfig } from "../../../src/config_service.ts";
...
export async function handleConfigRequest(req: Request): Promise<Response> {
  ...
  try {
    ...
    if (req.method === "GET") {
      const bundle = await withRequestContext(claims, (client) => fetchActiveConfig(client));
    ...
    if (req.method === "PATCH") {
      ...
      const bundle = await withRequestContext(claims, (client) => publishConfig(client, claims.tenant_id!, document));
      return withCors(jsonResponse(200, { data: bundle.document }, { etag: versionETag(bundle.version) }));
    }

    throw new AppError(405, "method_not_allowed", `${req.method} is not supported on /v1/config.`);
  } catch (err) {
...
if (import.meta.main) {
  Deno.serve(handleConfigRequest);
}
